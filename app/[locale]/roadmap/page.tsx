...
export default function RoadmapPage() {
  ...
  const [posts, setPosts] = useState<any[]>([]); // Converted to state to handle live data
  ...
  useEffect(() => {
    async function fetchSystemData() {
      ...
      try {
        ...
        if (roadmapError) {
          console.error("Error fetching roadmap items:", roadmapError);
        } else if (roadmapData) {
          setSteps(roadmapData);
        }

        // 2. Fetch Terminal Log Stories (Adjust table name 'posts' if yours is different)
        ...
        if (postsError) {
          console.error("Error fetching live logs:", postsError);
        } else if (postsData) {
          setPosts(postsData);
        }
      } catch (error) {
        console.error("Data stream exception:", error);
      } finally {
        setIsLoading(false);
      }
    }
  ...
  if (isLoading) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-zinc-50 dark:bg-zinc-950 font-mono text-emerald-500">
        <span className="animate-pulse">LOADING_SYSTEM_ARCHITECT_STREAM...</span>
      </div>
    );
  }
  ...
  return (
    <div className="relative min-h-screen bg-zinc-50 text-zinc-800 font-mono dark:bg-zinc-950 dark:text-zinc-300 transition-colors duration-300 overflow-x-hidden">
      ...
      <PageWrapper className="relative z-10 max-w-[1400px] mx-auto px-6 md:px-16 py-12">
        ...
        {/* Dashboard Content Split Grid (12-Column Grid Setup) */}
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
          
          {/* Panel 1: Left Navigation Roadmap Panel (3/12 Columns) */}
          <div className="lg:col-span-3 space-y-6">
            <div className="p-6 rounded-2xl border border-zinc-200 dark:border-white/5 bg-white/[0.6] dark:bg-white/[0.01] backdrop-blur-xl">
              ...
              <p className="text-xs text-zinc-500 dark:text-slate-500 bg-transparent">
          ...
          {/* Panel 2: Live Database Stories Feed Panel (4/12 Columns - Fixed layout constraint) */}
          <div className="lg:col-span-4 p-6 border border-zinc-200 dark:border-white/5 rounded-2xl bg-white/[0.6] dark:bg-white/[0.02] backdrop-blur-xl min-w-0">
          ...
          {/* Panel 3: Right Inspection / Terminal Details View Panel (5/12 Columns) */}
