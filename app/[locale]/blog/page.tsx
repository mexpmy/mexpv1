...
export default function BlogPage() {
  ...
  // NEW: Reference to the footer to trigger the modal perfectly
  ...
  // 1. Scroll listener just for the floating button (safe for mobile)
  useEffect(() => {
    const handleScroll = () => {
  ...
  // 2. Intersection Observer to trigger the modal ONLY when they hit the footer
  useEffect(() => {
    if (hasTriggeredRecommended) return;

    const observer = new IntersectionObserver(
      ([entry]) => {
        if (entry.isIntersecting && !hasTriggeredRecommended) {
          setHasTriggeredRecommended(true);
          // Wait half a second after they hit the bottom to smoothly open
          setTimeout(() => setIsRecommendedOpen(true), 500);
        }
      },
      { threshold: 0.1 } // Triggers when 10% of the footer is visible on screen
    );
  ...
  return (
    <div className="relative min-h-screen bg-zinc-50 text-zinc-800 font-mono selection:bg-emerald-500/20 dark:bg-zinc-950 dark:text-zinc-300 transition-colors duration-300 overflow-x-hidden">
      ...
      <PageWrapper className="relative z-10 max-w-[1400px] md:px-16 pb-12 mx-auto">
        ...
        {/* Footer Accent - NOW WITH REF */}
        <footer 
          ref={footerRef} 
          className="mt-32 pt-12 border-t border-zinc-200 dark:border-white/5 flex flex-col md:flex-row justify-between items-center gap-8 opacity-60 hover:opacity-100 transition-opacity"
        >
